								...
								<TR>
									<TD align=middle>
										<FONT style="FONT-SIZE: 10pt" face=宋体>合计 <SPAN
											...
											href="type.do?cur_page=${cur_page-1 }&typeid=${typeMessage.key.id }">上一页</A>
											</c:if>${cur_page == 0?"上一页":""} 
											<c:if test="${cur_page < page}">
											<A id=pageNext style="DISPLAY: inline-block; WIDTH: 45px; HEIGHT: 1px"
											href="type.do?cur_page=${cur_page+1 }&typeid=${typeMessage.key.id }">下一页</A>
											</c:if>
											${cur_page==page?"下一页":""} 
											<c:if test="${cur_page < page}">
											<A id=pageTail
											style="DISPLAY: inline-block; WIDTH: 32px; HEIGHT: 8px"
											href="type.do?cur_page=${page }&typeid=${typeMessage.key.id }">尾页</A>
